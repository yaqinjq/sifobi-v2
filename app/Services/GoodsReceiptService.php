...
class GoodsReceiptService
{
    ...
    public function createDraft(array $data, int $userId): GoodsReceipt
    {
        return DB::transaction(function () use ($data, $userId): GoodsReceipt {
            ...
            $receipt = GoodsReceipt::query()->create([
                ...
                'purchase_order_shipment_id' => $data['purchase_order_shipment_id'] ?? null,
    ...
    public function updateDraft(GoodsReceipt $receipt, array $data, int $userId): GoodsReceipt
    {
        return DB::transaction(function () use ($receipt, $data, $userId): GoodsReceipt {
            ...
            $receipt->update([
                ...
                'purchase_order_shipment_id' => $data['purchase_order_shipment_id'] ?? $receipt->purchase_order_shipment_id,
                'supplier_id' => $data['supplier_id'] ?? null,
                'supplier_name' => $supplierName,
                'vendor_name' => $supplierName,
                'doc_number' => $data['doc_number'] ?? null,
    ...
    public function postToLedger(GoodsReceipt $receipt, int $userId): void
    {
        ...
        // for why), so the outcome lands on the receipt asynchronously via the job's
        // handle()/failed() rather than here.
        if ($receipt->isWiproReceipt() && $receipt->purchaseOrder?->external_reference) {
            $receipt->forceFill(['receipt_synced_at' => null, 'receipt_sync_error' => null])->save();
            \App\Jobs\ConfirmReceiptToWiproJob::dispatch($receipt->id);
        }
    }
