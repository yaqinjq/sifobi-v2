...
<x-sf.page-header
    title="Kategori Menu"
    subtitle="Kelompok menu untuk tab kategori di POS"
...
<div class="px-4 py-5 lg:px-6 lg:py-6 max-w-5xl mx-auto w-full space-y-5">
    ...
    <x-sf.card title="Daftar Kategori Menu">
        ...
        <div class="divide-y divide-gray-50">
            @forelse($categories as $category)
                <div class="py-3" x-data="{ editing: false }">
                    <form method="POST" action="{{ route('settings.menu-categories.update', $category) }}" class="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr_1fr_1fr_auto] gap-3 items-center">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <span x-show="!editing" class="{{ $category->badgeClass() }}">{{ $category->color }}</span>
                            <select x-show="editing" x-cloak name="color" class="sf-input text-base">
                                @foreach($colors as $color)
                                    <option value="{{ $color }}" @selected($category->color === $color)>{{ $color }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <form method="POST" action="{{ route('settings.menu-categories.destroy', $category) }}" class="mt-2 flex justify-end">
                        @csrf
                        @method('DELETE')
                        <x-icon-btn
                            icon="delete"
                            ...
                            color="red"
                            type="submit"
                            onclick="return confirm('Yakin hapus kategori menu ini?')"
                        />
                    </form>
                </div>
            @empty
                <p class="text-sm text-gray-400 py-8 text-center">Belum ada kategori menu.</p>
            @endforelse
        </div>
    </x-sf.card>

    <x-sf.card title="+ Tambah Kategori Menu Baru">
        <form method="POST" action="{{ route('settings.menu-categories.store') }}" class="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr_1fr_1fr_auto] gap-3 items-end">
            @csrf
            ...
            <x-sf.form-group label="Nama" for="name" :required="true">
                <input id="name" name="name" value="{{ old('name') }}" class="sf-input text-base" maxlength="150" placeholder="Contoh: Minuman" required>
            </x-sf.form-group>
