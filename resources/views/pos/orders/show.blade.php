...
<div class="px-4 py-5 lg:px-6 lg:py-6 max-w-6xl mx-auto w-full space-y-5">
    ...
    @if($canEdit)
    <div class="lg:grid lg:grid-cols-[1fr_380px] lg:gap-5 lg:items-start space-y-5 lg:space-y-0">
    @else
    <div class="space-y-5">
    @endif

        {{-- ================= KATALOG (kiri di desktop) ================= --}}
        @if($canEdit)
        <div>
            <x-sf.card title="Menu" :padding="false">
                <div class="p-4"
                     x-data="{
                        search: '',
                        activeCategory: null,
                        menus: @js($menus->map(fn ($m) => [
                            ...
                            'placeholder_class' => $m->category?->placeholderClass() ?? 'ph-gray',
                        ...
                        get filtered() {
                            ...
                            return this.menus.filter(m =>
                                ...
                                (this.activeCategory === null || m.category_id === this.activeCategory)
                            );
                        }
                     }">
                    ...
                    @if($menuCategories->isNotEmpty())
                        <div class="flex gap-2 overflow-x-auto pb-1 mb-3">
                            <button type="button" @click="activeCategory = null"
                                    :class="activeCategory === null ? 'bg-primary-700 text-white' : 'bg-gray-100 text-gray-600'"
                                    class="shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors">
                                Semua
                            </button>
                            @foreach($menuCategories as $cat)
                                <button type="button" @click="activeCategory = {{ $cat->id }}"
                                        :class="activeCategory === {{ $cat->id }} ? 'bg-primary-700 text-white' : 'bg-gray-100 text-gray-600'"
                                        class="shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors">
                                    {{ $cat->name }}
                                </button>
                            @endforeach
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-3">
                            <template x-for="menu in filtered" :key="menu.id">
                                <form method="POST" action="{{ route('pos.orders.items.store', $order) }}">
                                    ...
                                    <button type="submit" class="w-full rounded-xl border border-gray-200 hover:border-primary-400 hover:shadow-card-hover overflow-hidden text-left transition-all bg-white">
                                        <div class="aspect-square w-full overflow-hidden" :class="!menu.photo ? menu.placeholder_class : ''">
                                            <img x-show="menu.photo" :src="menu.photo" class="w-full h-full object-cover" :alt="menu.name">
                                            <div x-show="!menu.photo" class="w-full h-full flex items-center justify-center text-3xl font-bold" x-text="menu.initial"></div>
                                        </div>
                                        <div class="p-2.5">
        ...
        {{-- ================= KERANJANG & PEMBAYARAN (kanan di desktop, sticky) ================= --}}
