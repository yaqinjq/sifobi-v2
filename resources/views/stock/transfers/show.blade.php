...
@section('content')
<x-sf.page-header title="Transfer Stok" subtitle="{{ $transfer->fromOutlet?->name }} &rarr; {{ $transfer->toOutlet?->name }}" back="{{ route('stock.transfers.index') }}" />
...
<div class="px-4 py-5 pb-24 lg:px-6 lg:py-6 max-w-3xl mx-auto w-full" x-data="{ rejectModal: false, voidModal: false }">
    ...
    <div class="flex gap-3">
        <form method="POST" action="{{ route('stock.transfers.submit', $transfer) }}" class="flex-1">
            @csrf
    ...
    <div class="flex gap-3">
        <form method="POST" action="{{ route('stock.transfers.approve', $transfer) }}" class="flex-1">
            @csrf
    ...
    <div x-show="rejectModal"
        ...
        <div @click.away="rejectModal = false"
            ...
            <form method="POST" action="{{ route('stock.transfers.reject', $transfer) }}">
                @csrf
    ...
    <div x-show="voidModal"
        ...
        <div @click.away="voidModal = false"
            ...
            <form method="POST" action="{{ route('stock.transfers.void', $transfer) }}">
                @csrf
